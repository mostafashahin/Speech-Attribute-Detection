...
affix = '_'.join(['Model',str(nhu),str(nhl),str(lr)])
...
for att in range(y_train.shape[1]):
    ...
    samples_train = min(MAX_TRAIN,index_train_p.shape[0],index_train_n.shape[0])
    samples_valid = min(MAX_VALID,index_valid_p.shape[0],index_valid_n.shape[0])
    samples_test = min(MAX_TEST,index_test_p.shape[0],index_test_n.shape[0])
    ...
    print('Start training attribute {0} with {1} p and {2} n training samples, {3} validation samples, {4} testing samples'.format(att,index_train_p.shape[0], index_train_n.shape[0], y_valid_att.shape[0],y_test_att.shape[0]), flush=True)
    mc = ModelCheckpoint(join('models_att_train_balanced','_'.join([str(att),'best_model.h5',affix])), monitor='val_loss', mode='min', save_best_only=True)
